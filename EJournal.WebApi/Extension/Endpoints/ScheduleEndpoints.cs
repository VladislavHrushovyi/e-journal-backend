...
public static class ScheduleEndpoints
{
    ...
    {
        group.MapPost("/add-work-day", [Authorize]
            async (AddWorkDayRequest req, IMediator mediatr, CancellationToken ct)
        ...
        group.MapPost("/update-work-time-status", [Authorize]
            async (ChangeStatusRequest req, IMediator mediator, CancellationToken ct)
                => await mediator.Send(req, ct));
        ...
        group.MapPatch("/set-work-day", async (UpdateIsWorkDayRequest req, IMediator mediator, CancellationToken ct)
            => await mediator.Send(req, ct));
        group.MapPatch("/delete-time", [Authorize]
            async ([FromBody] DeleteWorkTimeFromWorkDayRequest req, IMediator mediator, CancellationToken ct)
                => await mediator.Send(req, ct));
        return group;
    }
}